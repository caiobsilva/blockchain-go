...
//Block - defines the blockchain's nodes
type Block struct {
	Index     int
	Timestamp string
	Data      string
...
//Message - gets the client's data for the POST request
type Message struct {
	Data string
}

//Blockchain - instantiates the blockchain from the defined struct
...
func run() error {
	...
	s := &http.Server{
		...
		MaxHeaderBytes: 1 << 20, //x << y -> x*2, y times
	}
